...
export class DeleteMessageSelfUseCase {
  ...
  async execute(command: DeleteMessageSelfCommand) {
    try {
      ...
      this.checkIsUserChatMember(command.userId, chat.members);
      await this.checkIsMessageAlreadyDeletedForUser(command.messageId, command.userId);

      await this.messageRepository.hideMessageForUser(command.messageId, command.userId);
